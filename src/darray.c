...
void darray_push(DynamicArray* dynamic_array, void* element_ptr) {
  assert(dynamic_array);
  assert(element_ptr);
  void* push_pos = cds_lalloc(&dynamic_array->linear_allocator, dynamic_array->element_size);
  assert(push_pos);
  memcpy(push_pos, element_ptr, dynamic_array->element_size);
...
void darray_pop(DynamicArray* dynamic_array) {
  ...
  memset(dynamic_array->linear_allocator.current, 0, dynamic_array->element_size);
  dynamic_array->array_size -= 1;
}
